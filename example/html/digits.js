function Main() {
        this.cat = new CatsEye(config[0], config[1], config[2], w1, w2);
...
Main.prototype = {
    ...
    drawInput: function() {
        ...
        img.onload = () => {
            ...
            small.drawImage(img, 0, 0, img.width, img.height, 0, 0, 8, 8);
            var data = small.getImageData(0, 0, 8, 8).data;
            for (var i = 0; i < 8; i++) {
                for (var j = 0; j < 8; j++) {
                    var n = 4 * (i * 8 + j);
                    inputs[i * 8 + j] = (data[n + 0] + data[n + 1] + data[n + 2]) / 3;
            ...
            for (i=0; i<8*8; i++) inputs[i] = 255-inputs[i];
            for (i=0; i<8*8; i++) inputs[i] /= 255;
